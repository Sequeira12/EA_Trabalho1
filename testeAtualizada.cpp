...
{
    for (int i=0;i<N;i++){
        if(colunasP[i]!=cb[i]) return false;
        if(colunasT[i]!=ct[i]) return false;
    }
    return (QuadranteP[0] == qb[0] && QuadranteP[1] == qb[1] && QuadranteP[2] == qb[2] && QuadranteP[3] == qb[3] 
    && diagonaisP[0] == db[0] && diagonaisP[1] == db[1]);
}
...
void ConstroiMatriz(int linha, vector<int> &combination, vector<vector<int>> &vec)
{
    if(combination[linha]==1) diagonaisP[0]++;
    if(combination[N-linha-1]==1) diagonaisP[1]++;
